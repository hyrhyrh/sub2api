...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	...
	// Check if domain override applies
	override, hasOverride := GetDomainOverride(c.Request.Host)

	// Check cache first (only for non-override requests)
	if !hasOverride {
		cached := s.cache.Get()
		if cached != nil {
	...
	if hasOverride {
		settingsJSON = applyDomainOverrideToJSON(settingsJSON, override)
	}
...
func applyDomainOverrideToJSON(settingsJSON []byte, override DomainOverride) []byte {
	var m map[string]any
	if err := json.Unmarshal(settingsJSON, &m); err != nil {
		return settingsJSON
	}
	if override.Name != "" {
		m["site_name"] = override.Name
	}
	if override.Subtitle != "" {
		m["site_subtitle"] = override.Subtitle
	}
